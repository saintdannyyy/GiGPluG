...
export default function Stats() {
  ...
  return (
    <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 bg-primary-foreground py-8">
      {stats.map((stat, index) => (
        <Card
          key={index}
          className={`p-6 rounded-sm hover:shadow-lg transition-all duration-300 hover:-translate-y-1 bg-white`}
        >
          <div className="flex flex-row items-center text-center gap-3">
            <div className="p-5 bg-[#E7F0FA] dark:bg-gray-800 shadow-sm">
              {stat.icon}
            </div>
